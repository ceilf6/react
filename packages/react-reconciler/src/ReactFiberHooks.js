...
// FC 进入到 render 阶段的入口函数
export function renderWithHooks<Props, SecondArg>(
  current: Fiber | null,
  workInProgress: Fiber, // WIP FiberNode
  ...
  // 每一次执行函数组件之前，先清空状态
  // FiberNode.memoizedState 即当前 FiberNode 链表第一个设为 null 即清空链表
...
function mountWorkInProgressHook(): Hook {
  const hook: Hook = {
    memoizedState: null, // 存储hook自身状态数据 - 不同 hook 内容不同
    ...
    next: null, // 下一个 hook - 链表串联
  };

  // 最终 hook 对象要以 链表 形式串联
  // 判断当前 hook 是否为链表第一个
  if (workInProgressHook === null) {
    // This is the first hook in the list
    // 如果是头节点，就存储到 FiberNode 的 .memoizedState
...
function updateWorkInProgressHook(): Hook {
  ...
  // 更新 workInProgressHook 指向
  // 让 workInProgressHook 指向最新的 Hook
  let nextWorkInProgressHook: null | Hook; // 下一个要进行工作的 hook
  if (workInProgressHook === null) {
...
function mountState<S>(
  ...
  // 1. 创建 hook 对象，传入初始值 - 内部调用的 mountWorkInProgressHook
  const hook = mountStateImpl(initialState);

  // 2. 副作用相关，设置 hook.queue
  const queue = hook.queue;
